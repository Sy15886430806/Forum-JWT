...
public class WeatherServiceImpl implements WeatherService {
    ...
    public WeatherVO fetchFromCache(double longitude, double latitude) {
        JSONObject geo = this.decompressStringToJson(rest.getForObject(
                "https://nx6pg7aqwu.re.qweatherapi.com/geo/v2/city/lookup?location="+longitude+","+latitude+"&key="+key, byte[].class));
    ...
    private WeatherVO fetchFromAPI(int id, JSONObject location) {
        ...
        JSONObject now = this.decompressStringToJson(rest.getForObject(
                "https://nx6pg7aqwu.re.qweatherapi.com/v7/weather/now?location="+id+"&key="+key, byte[].class));
        if(now == null) return null;
        vo.setNow(now.getJSONObject("now"));
        JSONObject hourly = this.decompressStringToJson(rest.getForObject(
                "https://nx6pg7aqwu.re.qweatherapi.com/v7/weather/24h?location="+id+"&key="+key, byte[].class));
